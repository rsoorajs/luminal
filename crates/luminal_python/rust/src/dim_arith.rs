...
#[inline]
pub(crate) fn commutative_key(expr: &IntExpr) -> (usize, String) {
    (expr.len(), format!("{expr:?}"))
}

/// Order `(a, b)` so the canonically-smaller expression is first.
#[inline]
pub(crate) fn sort_pair(a: IntExpr, b: IntExpr) -> (IntExpr, IntExpr) {
...
#[inline]
pub(crate) fn mul_dims(a: IntExpr, b: IntExpr) -> IntExpr {
...
#[inline]
pub(crate) fn add_dims(a: IntExpr, b: IntExpr) -> IntExpr {
    let (a, b) = sort_pair(a, b);
    a + b
}
...
pub(crate) fn product_of_dims<I>(dims: I) -> IntExpr
where
    I: IntoIterator<Item = IntExpr>,
{
    ...
    v.into_iter()
        .fold(IntExpr::from(1usize), |acc, d| acc * d)
}
