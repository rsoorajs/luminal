...
pub fn parse_reduce_min_node(
    ...
    trace!("Starting parse: ReduceMin Node");
    assert!(
        !node.input.is_empty(),
        "ReduceMin should have at least 1 input"
    );
    assert!(
        node.output.len() == 1,
        "ReduceMin should have exactly 1 output"
    );

    let input = *tensors
        .get(&node.input[0])
        .ok_or_else(|| format!("ReduceMin: missing input tensor '{}'", node.input[0]))?;
    ...
    let raw_axes: Vec<i64> = if node.input.len() > 1 && !node.input[1].is_empty() {
        let axes_vals = known_values.get(&node.input[1]).ok_or_else(|| {
            format!(
                "ReduceMin: axes input '{}' must be a known constant",
                node.input[1]
            )
        })?;
        axes_vals.iter().map(|&v| v as i64).collect()
    ...
    let raw_axes: Vec<i64> = if raw_axes.is_empty() {
        if noop_with_empty_axes {
            tensors.insert(output_name.clone(), input);
            trace!("Finished parse: ReduceMin Node (noop)");
            return Ok(());
        } else {
            (0..ndim as i64).collect()
        }
    ...
    if normalized_axes.len() == ndim {
        // All-axes reduction: flatten to [1, N] and min axis 1 → [1].
        // luminal's Expression::product() returns 0 for empty iterators, so a MinReduce
        // producing a 0-dim tensor causes CUDA to launch with grid (0,1,1), which is
        // invalid. Using [1, N] → min(1) → [1] avoids this entirely.
        let total: usize = input_dims
            .iter()
            .map(|d| d.to_usize().expect("ReduceMin: dim must be concrete"))
            .product();
        let mut flat = input;
        flat.shape = ShapeTracker::new(vec![1, total]);
        let mut result = flat.min(1); // [1, N].min(1) → [1]
        ...
        tensors.insert(output_name.clone(), result);
        trace!("Finished parse: ReduceMin Node (all-axes)");
        return Ok(());
    }
    ...
    tensors.insert(output_name.clone(), result);
    trace!("Finished parse: ReduceMin Node");
    Ok(())
}
...
pub fn parse_reduce_mean_node(
    ...
    trace!("Starting parse: ReduceMean Node");
    assert!(
        !node.input.is_empty(),
        "ReduceMean should have at least 1 input"
    );
    assert!(
        node.output.len() == 1,
        "ReduceMean should have exactly 1 output"
    );
    ...
    let raw_axes: Vec<i64> = if node.input.len() > 1 && !node.input[1].is_empty() {
        let axes_vals = known_values.get(&node.input[1]).ok_or_else(|| {
            format!(
                "ReduceMean: axes input '{}' must be a known constant",
                node.input[1]
            )
        })?;
        axes_vals.iter().map(|&v| v as i64).collect()
    ...
    let raw_axes: Vec<i64> = if raw_axes.is_empty() {
        if noop_with_empty_axes {
            tensors.insert(output_name.clone(), input);
            trace!("Finished parse: ReduceMean Node (noop)");
            return Ok(());
        } else {
            (0..ndim as i64).collect()
        }
    ...
    if normalized_axes.len() == ndim {
        // All-axes reduction: flatten to [1, N] and mean axis 1 → [1].
        // luminal's Expression::product() returns 0 for empty iterators, so a SumReduce
        // producing a 0-dim tensor causes CUDA to launch with grid (0,1,1), which is
        // invalid. Using [1, N] → sum(1)/N → [1] avoids this entirely.
        // We also use Div<f32> instead of mean() (which uses Div<Expression>) because
        // the Div<Expression> path creates an Iota+Cast chain that triggers
        // CUDA_ERROR_INVALID_VALUE on CUDA backends.
        let total: usize = input_dims
            .iter()
            .map(|d| d.to_usize().expect("ReduceMean: dim must be concrete"))
            .product();
        let mut flat = input;
        flat.shape = ShapeTracker::new(vec![1, total]);
        let mut result = flat.sum(1) / (total as f32); // [1, N].sum(1)/N → [1]
        ...
        tensors.insert(output_name.clone(), result);
        trace!("Finished parse: ReduceMean Node (all-axes)");
        return Ok(());
    }
    ...
    tensors.insert(output_name.clone(), result);
    trace!("Finished parse: ReduceMean Node");
    Ok(())
}
