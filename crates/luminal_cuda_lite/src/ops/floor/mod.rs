...
/// `FloorFunctionalGeneric(input) -> out` — pure dataflow form.
...
/// Destination-passing form: `Floor(input: read, dest0: write ↔ out0)`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl KernelOp for FloorFunctionalDps {
    fn codegen(&self, ctx: &CodegenCtx) -> Result<Vec<KernelSource>> {
        unary(ctx, "floorf(a[i])")
    }
}

/// Matches `LayoutTensorOpFloorFunctionalGeneric` and produces this
/// runtime's [`FloorFunctional`].
