...
// Matches the logical Exp op from logical.rs, giving the search a
// single-kernel alternative to the 3-kernel Constant+Mul+Exp2 path.
// This improves numerical precision by avoiding the truncated log2(e) constant.
...
impl EgglogOp for KernelExp {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        vec![
            // Match via the logical Exp op (created by logical.rs Exp rewrite)
            kernel_rewrite::<crate::logical::Exp, Self>(),
            // Also match Exp2(Mul(x, log2e_constant)) directly, in case the logical
            // Exp rewrite doesn't fire (e.g., constant format mismatch).
...
// Matches the logical Sigmoid op from logical.rs, giving the search a
...
impl EgglogOp for KernelSigmoid {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        vec![
            // Match via the logical Sigmoid op (created by logical.rs Sigmoid rewrite)
            kernel_rewrite::<crate::logical::Sigmoid, Self>(),
            // Also match the HLIR pattern directly: Recip(Add(Exp2(Mul(Mul(x, -1), log2e)), 1))
            // This fires even if the logical Exp/Sigmoid rewrites don't fire.
            Rule::raw(
                "(rule
                (
