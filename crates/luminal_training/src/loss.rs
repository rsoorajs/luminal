...
pub fn huber_loss(
    ...
    let delta_tensor = prediction.graph().constant_float(delta);
    let huber_error = (0.5 * (prediction - target).square())
        * abs_error.lt(delta_tensor.expand_rhs(abs_error.shape))
        + (delta * (abs_error - 0.5 * delta))
            * abs_error.ge(delta_tensor.expand_rhs(abs_error.shape));
