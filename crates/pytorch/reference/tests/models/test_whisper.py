...
# Exercise the example belonging to the explicitly selected test backend.
EXAMPLE_PATH = (
    Path(__file__).resolve().parents[3] / selected_backend() / "examples" / "whisper.py"
)
_spec = importlib.util.spec_from_file_location(
    f"whisper_{selected_backend()}", EXAMPLE_PATH
)
assert _spec is not None and _spec.loader is not None
whisper_demo = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(whisper_demo)
