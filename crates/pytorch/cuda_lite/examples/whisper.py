"""Whisper transcription using the Luminal cuda_lite backend.
...
decoding through ``torch.compile`` with ``luminal_cuda_lite.Compiler``.

Usage::

    cd crates/pytorch/cuda_lite
    ...
    try:
        ...
        with (
            ...
            while chunk := response.read(65536):
                output.write(chunk)
        # Do not cache an HTTP error page or an incomplete/non-WAV response.
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("The CUDA Whisper example requires a CUDA GPU")
