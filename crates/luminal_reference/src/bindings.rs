//! The reference runtime's boundary bindings, hand-rolled.
//!
//! The model is boundary-free logical structure; which values enter and
//! leave, through which buffers, at which layout and under which contents
//! permission is this runtime's statement, made at load and rendered
//! here into the preamble's boundary vocabulary. Aliasing has exactly one
//! spelling: two bindings naming the same buffer id. Nothing else about
//! them need agree. Every buffer declares its access and who frees it;
//! the declarations are re-asserted after saturation as checks.
//!
//! Reference bindings are dense: row-major contiguous at the dtype's own
...
impl ReferenceBindings {
    /// The schedule every reference program runs: core rulesets saturate
