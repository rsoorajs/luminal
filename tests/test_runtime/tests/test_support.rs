...
mod harness_tests {
    ...
    use test_runtime::test_support::*;
    // These fixtures exercise the reference runtime vocabulary.
        ...
        for stem in GOLDEN_SCRIPTS {
            ...
            fs::write(
                std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                    .join("../../output")
                    .join(format!("{stem}.bufferized.txt")),
                plan.summary(),
            )
...
"#;
    ...
    fn multi_destination_pairs_get_distinct_allocations() {
        use luminal::bufferize::BufferNode;
        use test_runtime::test_support::test_ops::AddMulFused;
...
mod intcoordvar_probe {
    ...
    fn run_steps(egraph: &mut luminal::prelude::egglog::EGraph, steps: &[(&str, &str)]) {
    ...
    fn rejoin_divergence_probe() {
        for lead in [1usize, 2usize] {
            ...
            egraph
                .parse_and_run_program(None, &full)
                .expect("body loads");
            let sizes = |egraph: &mut luminal::prelude::egglog::EGraph| -> std::collections::HashMap<String, isize> {
                let out = egraph
            ...
            let channel_counts =
                |egraph: &mut luminal::prelude::egglog::EGraph| -> (usize, usize, usize) {
            ...
            for round in 1..=150 {
                ...
                for chunk in &round_out {
                    let luminal::prelude::egglog::CommandOutput::RunSchedule(report) = chunk else {
                        continue;
                    };
...
mod ring_ignition_battery {
    ...
    /// formats parsed.
    fn table_sizes(
        egraph: &mut luminal::prelude::egglog::EGraph,
    ) -> std::collections::BTreeMap<String, isize> {
        let out = egraph
            .parse_and_run_program(None, "(print-size)")
            .expect("sizes");
...
mod escape_execution_tests {
    ...
    fn rm_layout(dims: &[i64]) -> DecodedLayout {
        // Dep-world discipline: `luminal::layouts`, never `luminal::layouts` —
