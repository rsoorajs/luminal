//! FORKED from `luminal_reference::ops::select` — the TestRuntime owns its
//! whole op set outright and depends on no other runtime. The kernel is
//! not carried over: this runtime is plan-level and never executes.
...
impl BufferTensorIrOp for SelectFunctionalDps {
    fn label(&self) -> &str {
        "SelectFunctionalGeneric" // DPS forms keep the IR name
    }
...
/// Matches `LayoutTensorOpSelectFunctionalGeneric` enodes. Metadata
/// children: `out_layout` at child 3.
