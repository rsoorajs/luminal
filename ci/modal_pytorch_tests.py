"""Shared Modal setup for the backend-owned PyTorch test entry points."""
...
def create_app(backend: str, local_project_dir: Path):
    """Build a CPU reference or CUDA test app without launching remote work."""
    if backend not in {"reference", "cuda_lite"}:
        raise ValueError(f"Unknown PyTorch backend: {backend}")
    app = modal.App(f"luminal-{backend}-tests")

    DEFAULT_TIMEOUT = 2 * 60 * 60
    CUDARC_CUDA_VERSION = "12080"
    LOCAL_PROJECT_DIR = Path(local_project_dir).resolve()
    PROJECT_DIR = "/root/luminal/crates/pytorch"
    VENV_PATH = f"/root/.cache/luminal/uv-project-environments/luminal-{backend}"
    SRC_PATH = f"{PROJECT_DIR}/reference/python:{PROJECT_DIR}/cuda_lite/python:{PROJECT_DIR}/utils/tests:{PROJECT_DIR}"
    ...
    image = (
        modal.Image.from_registry(
            f"ghcr.io/luminal-ai/luminal-docker:{'cuda' if backend == 'cuda_lite' else 'cpu'}"
        )
        .env({"CUDARC_CUDA_VERSION": CUDARC_CUDA_VERSION})
        .workdir(PROJECT_DIR)
        .add_local_dir(
            str(LOCAL_PROJECT_DIR.parent.parent.parent),
        ...
        .run_commands(f"uv sync --project {backend} --group dev")
    )
    ...
    def _build_extensions(env: dict[str, str]) -> None:
        for extension in (
            ("reference", "cuda_lite") if backend == "cuda_lite" else ("reference",)
        ):
            cmd = [
                "uv",
                "run",
                ...
                "develop",
                "--manifest-path",
                f"{PROJECT_DIR}/{extension}/Cargo.toml",
                "--profile",
                "release",
            ]
    ...
    @app.cls(image=image, timeout=DEFAULT_TIMEOUT, serialized=True)
    class TestRunner:
        @modal.method()
        def run(
            self,
            ...
            env["LUMINAL_TEST_DEVICE"] = "cuda" if backend == "cuda_lite" else "cpu"
            env["LUMINAL_TEST_BACKEND"] = backend
            env["UV_PROJECT_ENVIRONMENT"] = VENV_PATH
            env["CUDARC_CUDA_VERSION"] = CUDARC_CUDA_VERSION
    ...
    def _parse_cli_args(
        ...
        parser = argparse.ArgumentParser(
            ...
            add_help=False,
            allow_abbrev=False,
            description=f"Run {backend} pytest coverage on Modal.",
        )
        parser.add_argument(
            "--gpu",
            required=backend == "cuda_lite",
        ...
        if not pytest_args:
            pytest_args = [f"{backend}/tests", "utils/tests"]
            if backend == "cuda_lite":
                pytest_args.append("reference/tests/models")
    ...
    def main(*cli_args: str):
        ...
        runner_options = {"gpu": gpu} if gpu else {}
        ...
        if result["profile_enabled"] and result["run_id"] is not None:
            if profile_output_dir:
                ...
                output_dir = _default_profile_output_dir(
                    result["local_default_dirname"]
                )
            ...
            try:
                ...
                print(
                    f"Failed to write local profile artifacts: {exc}", file=sys.stderr
                )
