...
int main(void) {
    ...
    for(int i=0;i<3;i++) {
        ...
        cuuint64_t keep=64*1024*1024; if(!check(cuMemPoolSetAttribute(pool,CU_MEMPOOL_ATTR_RELEASE_THRESHOLD,&keep),"set_threshold")) {cuMemPoolDestroy(pool);continue;}
        for(int trial=0;trial<2;trial++) {
            CUdeviceptr ptr=0; double t=now();
            if(!check(cuMemAllocFromPoolAsync(&ptr,1024*1024,pool,stream),"allocate")) break;
            ...
            if (i>0) {
                memset((void*)(uintptr_t)ptr,0x5a,1024*1024);
            ...
            printf("  trial=%d bytes=1048576 memory_type=%u alloc_sync_ms=%.3f free_sync_ms=%.3f retained=%llu\n",trial,type,alloc_ms,free_ms,(unsigned long long)reserved);
        }
        check(cuMemPoolDestroy(pool),"destroy");
    }
